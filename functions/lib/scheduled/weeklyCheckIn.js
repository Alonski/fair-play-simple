...
export const weeklyCheckIn = onSchedule({ schedule: 'every sunday 09:00', timeZone: 'Asia/Jerusalem' }, async () => {
    ...
    const partnerATime = partnerACards.reduce((s, c) => s + (c.metadata?.timeEstimate || 30), 0);
    const partnerBTime = partnerBCards.reduce((s, c) => s + (c.metadata?.timeEstimate || 30), 0);
    // Get recent stats
    ...
    const transfers = recentEvents.filter((e) => e.type === 'card_transferred').length;
    const assignments = recentEvents.filter((e) => e.type === 'card_assigned').length;
    // Generate summary
...
//# sourceMappingURL=weeklyCheckIn.js.map