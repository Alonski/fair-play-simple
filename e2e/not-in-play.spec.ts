...
test.describe('Not in Play', () => {
  ...
  test('clicking Remove hides card and shows Not in Play section', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /not in play/i })).toBeVisible();
  ...
  test('expanding Not in Play section shows Restore button', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /not in play/i }).click();
    // Expand the card inside it
    const notInPlayCardHeader = page.locator('[class*="border-l-4"]').first().getByRole('button').first();
  ...
  test('clicking Restore returns card to Unassigned', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /not in play/i })).not.toBeVisible();
  });
});
